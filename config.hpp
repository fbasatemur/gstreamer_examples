...
namespace GST_PARAMS{
    ...
    static std::string webcam_client_pipeline = std::string{"gst-launch-1.0 -vc udpsrc port=" + multicast_port + 
                                                            " close-socket=false multicast-iface=false auto-multicast=true ! application/x-rtp, payload=96 "
                                                            "! rtpjitterbuffer ! rtph264depay ! avdec_h264 ! fpsdisplaysink  sync=false --verbose"};

    static const std::string video_path{"../video/g_video.mp4"};

    static std::string filesrc_server_pipeline = std::string{"gst-launch-1.0 filesrc location=" + video_path + 
